...
class AutoSeAMv2_PromptEncoder(nn.Module):
    def __init__(
            self,
            embed_dim: int,
        ...
        self.mask_downscaling = nn.Sequential(
            nn.Conv2d(class_num + 1, mask_in_chans // 4, kernel_size=3, stride=1,padding=1),  #  stride=1,padding=1
            LayerNorm2d(mask_in_chans // 4),
            activation(),
            nn.Conv2d(mask_in_chans // 4, mask_in_chans, kernel_size=3, stride=1,padding=1),  #  stride=1,padding=1
...
if __name__ == '__main__':
    prompt_encoder = AutoSeAMv2_PromptEncoder(embed_dim=128, image_embedding_size=(32, 32), input_image_size=(512, 512),
    ...
    points = None
    box = None
    mask = torch.randn((2, 2, 13, 256, 256))
    query = torch.randn((2, 128, 32, 32))
    key = torch.randn((2, 2, 128, 32, 32))
    sparse_embeddings, dense_embeddings = prompt_encoder(points, box, mask, query, key, 'semantic_query')
