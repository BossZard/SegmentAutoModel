    ...
    def __init__(self, sam_model: Sam, r: int, fact_layer=None, s=1):  # s是尺度系数
        ...
        for t_layer_i, blk in enumerate(sam_model.image_encoder.blocks):
            if t_layer_i not in self.fact_layer:
                continue
            # Low Rank Adaptor in attention qkv
...
class Fact_tt_AutoSeAMv2(nn.Module):
    """Applies low-rank adaptation to a Sam model's image encoder.
    ...
    def __init__(self, sam_model: Sam, r: int, fact_layer=None, s=1):  # s是尺度系数
        super(Fact_tt_AutoSeAMv2, self).__init__()
    ...
    def forward(self, batched_input, box_prompts_inp, point_prompts_inp, mask_prompts_inp, dense_prompt_type, multimask_output,
                image_size):
        return self.sam(batched_input, box_prompts_inp, point_prompts_inp, mask_prompts_inp, dense_prompt_type, multimask_output,
                        image_size)
